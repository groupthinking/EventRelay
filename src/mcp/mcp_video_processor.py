...
try:
    from shared.libs.youtube_proxy import create_youtube_proxy
...
class MCPErrorHandler:
    ...
    def handle_timeout_error(
        self, operation: str, timeout_seconds: int, video_id: str = None
    ) -> TimeoutError:
        ...
        self.logger.error(
            error_msg, video_id=video_id, operation=operation, timeout=timeout_seconds
        )
        return TimeoutError(error_msg)

    def handle_circuit_breaker_error(
        self, breaker_name: str, video_id: str = None
    ) -> CircuitBreakerError:
    ...
    def handle_transcript_error(
        self, error: Exception, video_id: str, method: str
    ) -> None:
        """Handle transcript extraction errors"""
        self._increment_error_count("transcript")
        self.logger.warning(
            f"Transcript extraction failed using {method}",
            error=error,
            video_id=video_id,
            method=method,
        )
    ...
    def handle_processing_error(
        self, error: Exception, video_id: str, stage: str
    ) -> None:
...
class MCPConfig:
    ...
    def get_optimal_provider(self, task_complexity: str = "medium") -> str:
        ...
        fallback_chain = routing_config.get(
            "fallback_chain", ["groq", "anthropic", "openai"]
        )
...
class ActionGenerationStrategy:
    ...
    def generate_actions(
        self, text: str, transcript: list[dict], ai_insights: dict | None = None
    ) -> list[dict[str, Any]]:
            ...
            actions.append(
                {
                    ...
                    "description": ai_insights.get(
                        "key_moments", "No key moments provided."
                    ),
                    "priority": "high",
                    "gemini_enhanced": True,
                }
            ...
            actions.append(
                {
                    ...
                    "description": ai_insights.get(
                        "objects", "No object detection provided."
                    ),
                    "priority": "medium",
                    "gemini_enhanced": True,
                }
...
class MCPVideoProcessor:
    ...
    def __init__(self, config_path: str = None):
        ...
        self.transcript_breaker = CircuitBreaker(
            "transcript", failure_threshold=3, recovery_timeout=60
        )
        self.processing_breaker = CircuitBreaker(
            "processing", failure_threshold=2, recovery_timeout=30
        )
        ...
        self.mcp_logger.info(
            "🚀 MCP Video Processor initialized with HANGING PROTECTION"
        )
    ...
    async def extract_transcript_mcp(self, video_id: str) -> list[dict[str, Any]]:
        ...
        if force_proxy and HAS_YT_PROXY:
            ...
            if yt_key:
                try:
                    self.mcp_logger.info(
                        "🔁 Using proxy-only mode for transcript", video_id=video_id
                    )
                    proxy = create_youtube_proxy(yt_key)
                    transcript = await proxy.get_transcript(video_id)
                    if transcript and len(transcript) > 0:
                        self.mcp_logger.info(
                            "✅ Proxy-only extraction successful",
                    ...
                    self.mcp_logger.warning(
                        "⚠️ Proxy-only extraction failed", error=e, video_id=video_id
                    )
            else:
                self.mcp_logger.warning(
                    "⚠️ Proxy-only requested but YOUTUBE_API_KEY missing",
                    video_id=video_id,
                )
        ...
        try:
            ...
            self.mcp_logger.error(
                "🚨 Direct extraction timeout", error=e, video_id=video_id
            )
        ...
        if HAS_YT_PROXY:
            ...
            if yt_key:
                try:
                    self.mcp_logger.info(
                        "🔁 Trying proxy fallback for transcript", video_id=video_id
                    )
                    proxy = create_youtube_proxy(yt_key)
                    transcript = await proxy.get_transcript(video_id)
                    if transcript and len(transcript) > 0:
                        self.mcp_logger.info(
                            "✅ Proxy fallback extraction successful",
                            segments=len(transcript),
                            video_id=video_id,
                        )
                        return transcript
                    ...
                    self.mcp_logger.warning(
                        "⚠️ Proxy fallback failed", error=e, video_id=video_id
                    )
            else:
                self.mcp_logger.warning(
                    "⚠️ Proxy fallback available but YOUTUBE_API_KEY missing",
                    video_id=video_id,
                )

        # All extraction methods failed - raise appropriate exception
        self.mcp_logger.error(
            "❌ All transcript extraction methods failed", video_id=video_id
        )
        self.processing_stats["failures"] += 1
        raise Exception(
            f"Unable to extract transcript for video {video_id}: All methods (direct, MCP routing, proxy fallback) failed"
        )

    @timeout_protection(timeout_seconds=PROCESSING_TIMEOUT)
    async def generate_actions_mcp(
        self,
        video_id: str,
        ...
        async def _process_content():
            ...
            try:
                ...
                if gemini_integration:
                    ...
                    gemini_enhanced_analysis = (
                        await gemini_integration.enhanced_video_analysis(
                            full_text, ai_context
                        )
                    )

                    logger.info(
                        f"✅ Gemini enhancement score: {gemini_enhanced_analysis.get('mcp_enhancement_score', 0):.2f}"
                    )
            ...
            base_actions = ActionGenerationFactory.get_strategy(
                category
            ).generate_actions(full_text, transcript, ai_insights)

            # Enhance actions with Gemini insights
            enhanced_actions = base_actions.copy()

            if gemini_enhanced_analysis and gemini_enhanced_analysis.get(
                "function_calls"
            ):
            ...
            if gemini_enhanced_analysis and gemini_enhanced_analysis.get(
                ...
                ):  # Only add high-confidence thinking insights
                    enhanced_actions.append(
                        {
                            "type": "gemini_reasoning",
                            "title": "AI Reasoning & Analysis",
                            "description": (
                                thinking["thought_process"][:100] + "..."
                                if len(thinking["thought_process"]) > 100
                                else thinking["thought_process"]
                            ),
                            "priority": "medium",
            ...
            if ai_insights:
                if ai_insights.get("function_call_suggestion"):
                    ...
                    logger.info(
                        f"⚙️ Processing legacy function call: {function_call_data.get('name')}"
                    )
                    enhanced_actions.append(
                        {
            ...
            if gemini_enhanced_analysis:
                ...
                gemini_savings = self._calculate_gemini_savings(
                    gemini_enhanced_analysis
                )
            ...
            result = {
                ...
                "estimated_duration": (
                    transcript[-1].get("start", 0) if transcript else 0
                ),
                ...
                "processing_quality": (
                    "gemini_enhanced"
                    if gemini_enhanced_analysis
                    else "enterprise_grade"
                ),
                "gemini_integration": {
                    "enabled": gemini_integration is not None,
                    "enhancement_score": (
                        gemini_enhanced_analysis.get("mcp_enhancement_score", 0.0)
                        if gemini_enhanced_analysis
                        else 0.0
                    ),
                    "function_calls_count": (
                        len(gemini_enhanced_analysis.get("function_calls", []))
                        if gemini_enhanced_analysis
                        else 0
                    ),
                    "thinking_confidence": (
                        gemini_enhanced_analysis.get("thinking_result", {}).get(
                            "confidence", 0.0
                        )
                        if gemini_enhanced_analysis
                        else 0.0
            ...
            logger.info(
                f"🎯 Enhanced MCP processing complete - {len(enhanced_actions)} actions generated"
            )
            return result
    ...
    def _detect_category(self, text: str) -> str:
        ...
        self.mcp_logger.info(
            "📊 MCP category detection", category=detected_category, scores=scores
        )
    ...
    def _calculate_cost_savings(self, provider: str, text_length: int) -> float:
        ...
        baseline_cost = (
            costs["openai"] * token_thousands
        )  # Compare against most expensive
    ...
    def _calculate_gemini_savings(self, gemini_analysis: dict[str, Any]) -> float:
        ...
        thinking_tokens = gemini_analysis.get("thinking_result", {}).get(
            "token_count", 0
        )
        ...
        thinking_savings = min(
            thinking_tokens / 1000 * 0.0005, 0.01
        )  # Max $0.01 savings

        # 3. Quality enhancement reducing rework
        quality_savings = enhancement_score * 0.005  # Up to $0.005 for high quality

        total_gemini_savings = (
            function_call_savings + thinking_savings + quality_savings
        )

        return round(total_gemini_savings, 4)

    async def save_results_mcp(
        self, video_id: str, content: dict[str, Any], processing_time: float
    ) -> dict[str, Any]:
    ...
    async def _process_action_generation_stage(
        self,
        video_id: str,
        ...
        return await self.generate_actions_mcp(
            video_id, transcript_data, optimal_provider, ai_insights
        )

    async def _process_save_stage(
        self, video_id: str, actionable_content: dict[str, Any], processing_time: float
    ) -> dict[str, Any]:
        """Stage 3: Save with MCP metadata"""
        logger.info(f"💾 Stage 3: Saving results for {video_id}")
        return await self.save_results_mcp(
            video_id, actionable_content, processing_time
        )
    ...
    def _build_success_result(
        ...
        video_id: str,
        transcript_data: list[dict],
        actionable_content: dict[str, Any],
    ...
    def _build_error_result(
        self, video_id: str, error: Exception, processing_time: float
    ) -> dict[str, Any]:
    ...
    async def process_video_mcp(
        ...
        try:
            ...
            actionable_content = await self._process_action_generation_stage(
                video_id, transcript_data, ai_insights
            )

            # Stage 3: Save results
            processing_time = time.time() - start_time
            save_result = await self._process_save_stage(
                video_id, actionable_content, processing_time
            )
            ...
            logger.info(
                f"✅ MCP processing complete: {video_id} in {processing_time:.3f}s"
            )
            logger.info(
                f"💰 Cost savings: ${actionable_content.get('cost_savings', 0.0):.4f}"
            )
    ...
    def get_processing_stats(self) -> dict[str, Any]:
        ...
        total_protected = (
            self.processing_stats["timeouts"]
            + self.processing_stats["circuit_breaker_trips"]
        )
        protection_rate = (
            total_protected / max(1, self.processing_stats["total_processed"])
        ) * 100
        ...
        return {
            **self.processing_stats,
            ...
            "average_savings_per_video": round(
                self.processing_stats["cost_savings"]
                / max(1, self.processing_stats["successful"]),
                4,
            ),
...
if HAS_MCP:
    ...
    @mcp.tool(
        name="process_video_with_ai_insights",
        description="Process a video with AI insights from Chrome AI API.",
    )
    async def process_video_with_ai_insights_tool(
        video_url: str, video_id: str, ai_insights: dict, audio_transcription: str
    ) -> dict:
        processor = MCPVideoProcessor()
        return await processor.process_video_mcp(
            video_url, ai_insights=ai_insights, audio_transcription=audio_transcription
        )
