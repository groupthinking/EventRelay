...
class MarkdownVideoProcessor:
    ...
    def __init__(self):
        ...
        prompt_path = (
            Path(__file__).parent.parent / "prompts" / "video_analysis_markdown.txt"
        )
    ...
    async def get_video_metadata(self, video_id: str) -> dict[str, Any]:
        ...
        if transcript_data:
            ...
            transcript_summary = " ".join(
                [seg.get("text", "") for seg in first_segments]
            )
            transcript_summary = (
                transcript_summary[:300] + "..."
                if len(transcript_summary) > 300
        ...
        async with aiohttp.ClientSession() as session:
            ...
            payload = {
                ...
                "messages": [
                    {
                        "role": "system",
                        "content": "You are an expert at creating educational content from videos.",
                    },
    ...
    def categorize_video(self, metadata: dict[str, Any]) -> str:
        ...
        if any(
            word in combined
            for word in ["learn", "tutorial", "course", "how to", "explained"]
        ):
            return "Educational"
        elif any(
            word in combined for word in ["code", "programming", "software", "dev"]
        ):
            return "Technology"
    ...
    async def process_video(self, video_url: str) -> dict[str, Any]:
        ...
        if comments:
            metadata["top_comments"] = [
                f"{c['text']} — {c['likes']} likes" for c in comments[:3]
            ]
        ...
        # Save results
        save_path = await self._save_markdown_result(
    ...
    async def _save_markdown_result(
        self, video_id: str, metadata: dict[str, Any], markdown: str
    ) -> str:
