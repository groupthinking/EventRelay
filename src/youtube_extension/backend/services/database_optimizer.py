    ...
    async def initialize(self):
        ...
        try:
            if HAS_POSTGRESQL and "postgresql" in self.database_url:
                ...
                logger.info(
                    f"✅ PostgreSQL connection pool initialized ({self.min_connections}-{self.max_connections} connections)"
                )
            else:
        ...
        try:
            ...
            with self._lock:
                self.pool_stats["connections_in_use"] = max(
                    0, self.pool_stats["connections_in_use"] - 1
                )
    ...
    def get_pool_stats(self) -> dict[str, Any]:
        ...
        return {
            **self.pool_stats,
            ...
            "recent_connection_times": list(self.connection_history)[
                -50:
            ],  # Last 50 connection times
        }
...
class QueryOptimizer:
    ...
    async def execute_query(
        ...
        if use_cache:
            ...
            if cached_result is not None:
                ...
                logger.debug(
                    f"Query cache HIT ({execution_time:.2f}ms): {query_pattern}"
                )

                # Update stats for cache hit (very fast execution)
                await self._update_query_stats(
                    query_hash, query_pattern, execution_time, True
                )
        ...
        try:
            ...
            if use_cache and execution_time > 50:  # Cache slower queries
                await cache_set(
                    cache_key, result, ttl=self.cache_ttl, tags=["database_query"]
                )
            ...
            execution_time = (time.time() - start_time) * 1000
            await self._update_query_stats(
                query_hash, query_pattern, execution_time, False
            )

            logger.error(
                f"Query failed ({execution_time:.2f}ms): {query_pattern} - {e}"
            )
            raise
    ...
    async def execute_batch_queries(
        ...
        try:
            ...
            for pattern, group_queries in query_groups.items():
                ...
                # Avoid N+1 sequential execution by always using asyncio.gather for batch queries
                # Since execute_query handles its own connection pooling/execution,
                # running them concurrently gives the best performance boost
                # without reinventing executemany constraints (e.g. lack of return values, grouping by identical SQL)
                coroutines = [
    ...
    async def _update_query_stats(
        ...
        if query_hash not in self.query_stats:
            self.query_stats[query_hash] = QueryStats(
                query_hash=query_hash, query_pattern=query_pattern
            )

        self.query_stats[query_hash].update_stats(execution_time_ms, success)

    async def _handle_slow_query(
        self, query: str, pattern: str, execution_time_ms: float
    ):
    ...
    async def get_performance_report(self) -> dict[str, Any]:
        ...
        avg_execution_times = [
            stats.avg_execution_time_ms for stats in self.query_stats.values()
        ]
        ...
        sub_100ms_percentage = (
            (sub_100ms_queries / total_queries * 100) if total_queries > 0 else 0
        )

        overall_avg_time = (
            statistics.mean(avg_execution_times) if avg_execution_times else 0
        )
        ...
        slow_queries = [
            {
                ...
                "error_rate": (
                    (stats.error_count / stats.execution_count * 100)
        ...
        return {
            ...
            "performance_summary": {
                ...
                "targets_met": {
                    "sub_100ms_target": sub_100ms_percentage
                    >= self.performance_targets["sub_100ms_percentage"],
                    "avg_time_target": overall_avg_time
                    <= self.performance_targets["avg_query_time_ms"],
                },
            },
            "detailed_metrics": {
            ...
            "recommendations": {
                "index_recommendations": [
                    asdict(rec) for rec in self.index_recommendations[-5:]
                ],  # Latest 5 recommendations
                "optimization_priority": (
                    "high"
                    if sub_100ms_percentage < 90
                    else "medium" if sub_100ms_percentage < 95 else "low"
                ),
                "next_actions": self._generate_next_actions(
                    sub_100ms_percentage, overall_avg_time
                ),
            },
    ...
    def _generate_next_actions(
        self, sub_100ms_percentage: float, avg_time_ms: float
    ) -> list[str]:
        ...
        if sub_100ms_percentage < 95:
            actions.append(
                f"Optimize slow queries - currently {sub_100ms_percentage:.1f}% under 100ms (target: 95%)"
            )

        if avg_time_ms > 50:
            actions.append(
                f"Reduce average query time from {avg_time_ms:.1f}ms to under 50ms"
            )

        if len(self.index_recommendations) > 0:
            actions.append(
                f"Implement {len(self.index_recommendations)} pending index recommendations"
            )

        if not actions:
            actions.append(
                "Performance targets met - continue monitoring and maintain current optimization level"
            )
...
class DatabaseHealthMonitor:
    ...
    async def run_health_check(self) -> dict[str, Any]:
        ...
        try:
            ...
            pool_usage = (
                pool_stats["connections_in_use"]
                / self.query_optimizer.connection_pool.max_connections
            ) * 100
            ...
            if self.query_optimizer.query_stats:
                ...
                slow_query_percentage = (
                    (slow_query_count / len(self.query_optimizer.query_stats) * 100)
                    if self.query_optimizer.query_stats
                    else 0
                )
                ...
                health_status["checks"]["query_performance"] = {
                    "status": (
                        "healthy"
                        if overall_avg_time < 100 and slow_query_percentage < 5
                        else "warning"
                    ),
                ...
                if (
                    slow_query_percentage
                    >= self.alert_thresholds["slow_query_percentage"]
                ):
                    health_status["alerts"].append(
                        {
                            "severity": "critical",
            ...
            return {
                ...
                "checks": {},
                "alerts": [
                    {"severity": "critical", "message": f"Health check failed: {e}"}
                ],
                "recommendations": [
