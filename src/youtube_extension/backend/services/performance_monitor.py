...
class PerformanceMonitor:
    ...
    async def record_metrics(self, metrics: Sequence[Mapping[str, Any]]) -> None:
        ...
        try:
            ...
            # whole batch would give every row an identical timestamp, which
            # diverges from the promised serial semantics and erases per-sample
            # ordering for callers that submit genuinely distinct samples.
            records = [
                PerformanceMetric(
