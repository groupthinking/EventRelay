...
class ProjectCodeGenerator:
    ...
    async def _generate_vanilla_js_project(self, project_path: Path, video_analysis: dict, features: list[str]) -> dict[str, Any]:
        ...
        styles_css = self._generate_vanilla_styles_css(features, fingerprint, title)
...
{extra_functions}'''
    ...
    def _generate_vanilla_styles_css(self, features: list[str], fingerprint: str = "", title: str = "") -> str:
...
@media (max-width: 768px) {
    ...
    .welcome {
        ...
        return f'''/* Generated by UVAI — accent derived from video fingerprint{title_comment} */
:root {{
    --accent: {accent};
...
*Generated with ❤️ by UVAI from [{video_url}]({video_url})*'''
    ...
    def _build_generation_context(self, video_analysis: dict[str, Any], project_config: dict[str, Any]) -> dict[str, Any]:
        ...
        # Extract summary and key_concepts early (needed for both paths)
        summary = self._extract_summary(ai_analysis, video_analysis)
        key_concepts = self._coerce_to_list(
            ai_analysis.get("Key Concepts")
            or extracted_info.get("key_concepts")
            or video_analysis.get("key_concepts")
        )
        ...
        if build_plan:
            ...
            title = build_plan.get("title") or "UVAI Generated Project"
            ...
            features = self._coerce_to_list(
                extracted_info.get("features")
                or ai_analysis.get("Features")
            )
