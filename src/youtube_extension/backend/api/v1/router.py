...
def _safe_log(value: object) -> str:
    """Return ``value`` as a string safe to interpolate into a log message.
    ...
    them — can smuggle CR/LF or terminal escapes to forge log records.
    Reuses the central ``sanitize_log_record`` table so call-site and
    formatter-level sanitization stay consistent.
    """
    from youtube_extension.backend.config.logging_config import sanitize_log_record

    return sanitize_log_record(str(value))
