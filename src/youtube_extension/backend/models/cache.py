...
class CacheEntry(BaseModel):
    ...
    cache_key: Mapped[str] = mapped_column(
        String(500), nullable=False, unique=True, index=True, doc="Unique cache key"
    )

    cache_type: Mapped[CacheType] = mapped_column(
        Enum(CacheType), nullable=False, index=True, doc="Type of cached content"
    )

    # Cache content
    content: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True, doc="Cached content (for text data)"
    )

    structured_data: Mapped[dict[str, Any]] = mapped_column(
        JSONB, default={}, nullable=False, doc="Structured cached data"
    )
    ...
    size_bytes: Mapped[int] = mapped_column(
        BigInteger, default=0, nullable=False, doc="Cache entry size in bytes"
    )

    content_type: Mapped[Optional[str]] = mapped_column(
        String(100), nullable=True, doc="MIME type of cached content"
    )

    compression_ratio: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True, doc="Compression ratio if content is compressed"
    )
    ...
    expires_at: Mapped[datetime] = mapped_column(
        nullable=False, index=True, doc="Cache expiration timestamp"
    )

    # Usage tracking
    hit_count: Mapped[int] = mapped_column(
        Integer, default=0, nullable=False, doc="Number of cache hits"
    )

    last_accessed_at: Mapped[Optional[datetime]] = mapped_column(
        nullable=True, index=True, doc="Last access timestamp"
    )

    access_frequency: Mapped[float] = mapped_column(
        Float, default=0.0, nullable=False, doc="Access frequency (hits per day)"
    )

    # Source information
    source_url: Mapped[Optional[str]] = mapped_column(
        String(1000), nullable=True, doc="Original source URL"
    )

    source_version: Mapped[Optional[str]] = mapped_column(
        String(50), nullable=True, doc="Source version or etag"
    )

    generation_time_ms: Mapped[Optional[int]] = mapped_column(
        Integer, nullable=True, doc="Time taken to generate content (ms)"
    )

    generation_cost: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True, doc="Cost to generate this content"
    )

    # Cache warming and maintenance
    warm_after: Mapped[Optional[datetime]] = mapped_column(
        nullable=True, doc="When to start warming this cache entry"
    )

    warming_priority: Mapped[int] = mapped_column(
        Integer, default=5, nullable=False, doc="Cache warming priority (1=highest)"
    )
    ...
    error_count: Mapped[int] = mapped_column(
        Integer, default=0, nullable=False, doc="Number of errors accessing this entry"
    )
...
class CacheStats(BaseModel):
    ...
    stats_date: Mapped[datetime] = mapped_column(
        nullable=False, unique=True, index=True, doc="Date for these statistics"
    )

    # Cache metrics
    total_entries: Mapped[int] = mapped_column(
        Integer, default=0, nullable=False, doc="Total cache entries"
    )

    active_entries: Mapped[int] = mapped_column(
        Integer, default=0, nullable=False, doc="Active cache entries"
    )

    expired_entries: Mapped[int] = mapped_column(
        Integer, default=0, nullable=False, doc="Expired cache entries"
    )

    # Size metrics
    total_size_bytes: Mapped[BigInteger] = mapped_column(
        BigInteger, default=0, nullable=False, doc="Total cache size in bytes"
    )

    average_entry_size: Mapped[float] = mapped_column(
        Float, default=0.0, nullable=False, doc="Average cache entry size"
    )

    largest_entry_size: Mapped[BigInteger] = mapped_column(
        BigInteger, default=0, nullable=False, doc="Largest cache entry size"
    )

    # Performance metrics
    hit_rate: Mapped[float] = mapped_column(
        Float, default=0.0, nullable=False, doc="Cache hit rate percentage"
    )

    miss_rate: Mapped[float] = mapped_column(
        Float, default=0.0, nullable=False, doc="Cache miss rate percentage"
    )

    total_hits: Mapped[BigInteger] = mapped_column(
        BigInteger, default=0, nullable=False, doc="Total cache hits"
    )

    total_misses: Mapped[BigInteger] = mapped_column(
        BigInteger, default=0, nullable=False, doc="Total cache misses"
    )

    # Timing metrics
    average_response_time_ms: Mapped[float] = mapped_column(
        Float, default=0.0, nullable=False, doc="Average cache response time"
    )

    time_saved_hours: Mapped[float] = mapped_column(
        Float, default=0.0, nullable=False, doc="Estimated time saved by caching"
    )

    # Cost metrics
    generation_cost_saved: Mapped[float] = mapped_column(
        Float, default=0.0, nullable=False, doc="Cost saved through caching"
    )

    storage_cost: Mapped[float] = mapped_column(
        Float, default=0.0, nullable=False, doc="Cost of cache storage"
    )

    # Cache type breakdown
    type_breakdown: Mapped[dict[str, Any]] = mapped_column(
        JSONB, default={}, nullable=False, doc="Statistics by cache type"
    )

    # Top performing caches
    top_entries: Mapped[list[dict[str, Any]]] = mapped_column(
        JSONB, default=[], nullable=False, doc="Top performing cache entries"
    )

    # Cleanup metrics
    entries_cleaned: Mapped[int] = mapped_column(
        Integer, default=0, nullable=False, doc="Cache entries cleaned up"
    )

    cleanup_size_freed: Mapped[BigInteger] = mapped_column(
        BigInteger, default=0, nullable=False, doc="Space freed by cleanup"
    )
...
Index(
    "ix_cache_entries_frequency_hits", CacheEntry.access_frequency, CacheEntry.hit_count
)
Index(
    "ix_cache_entries_warm_priority", CacheEntry.warm_after, CacheEntry.warming_priority
)
