...
async def process_video_cloud(
    ...
    try:
        ...
        safe_url = request.video_url.replace('\r', '').replace('\n', '')
        logger.info(
            f"🎬 Cloud processing request: {safe_url} "
        ...
        error_msg = "Cloud processing failed due to an internal error."
...
async def process_video_task_handler(
    ...
    safe_task_name = x_cloudtasks_taskname.replace('\r', '').replace('\n', '')
    safe_video_id = payload.video_id.replace('\r', '').replace('\n', '')
    logger.info(
        f"📝 Processing Cloud Task: {safe_task_name} "
    ...
    try:
        ...
        if payload.callback_url and result.success:
            ...
            # Restrict to https scheme and disallow local loopback for external callbacks
            if parsed_url.scheme != 'https' or parsed_url.hostname in ('localhost', '127.0.0.1', '0.0.0.0'):
                logger.warning("Invalid callback URL scheme or hostname provided.")
            else:
                try:
                    import httpx
                    ...
                    # Use sanitized callback url in log
                    safe_url = payload.callback_url.replace('\r', '').replace('\n', '')
                    logger.info(f"✅ Callback sent to {safe_url}")
                except Exception as e:
                    logger.warning(f"⚠️ Callback failed for {payload.callback_url.replace('\r', '').replace('\n', '')}")
        ...
        error_msg = "Task processing failed due to an internal error."
...
async def get_video_status(video_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Error retrieving status due to an internal error."
        )
...
async def get_video_result(video_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Error retrieving result due to an internal error."
        )
