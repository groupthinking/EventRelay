...
class YouTubeInnovationLearningDB:
    ...
    def __init__(self, db_path: str = "logs/youtube_innovation_learning.db") -> None:
    ...
    async def get_innovation_dashboard(self, args: dict[str, Any]) -> dict[str, Any]:
        ...
        # Convert time range to ISO timestamp string
        now = datetime.now(timezone.utc)
        time_filters = {
            "24h": (now - timedelta(days=1)).strftime("%Y-%m-%d %H:%M:%S"),
            "7d": (now - timedelta(days=7)).strftime("%Y-%m-%d %H:%M:%S"),
            "30d": (now - timedelta(days=30)).strftime("%Y-%m-%d %H:%M:%S"),
            "all": "2020-01-01 00:00:00",
        }
