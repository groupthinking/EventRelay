...
const ExampleGallery_1 = __importDefault(require("@/components/ExampleGallery"));
...
function App() {
    const { defaultExample, examples, setExamples, isLoading } = (0, react_1.useContext)(context_1.DataContext);
    const [videoUrl, setVideoUrl] = (0, react_1.useState)(PRESEED_CONTENT ? defaultExample?.url : '');
    const [urlValidating, setUrlValidating] = (0, react_1.useState)(false); // State to track URL validation
    const [contentLoading, setContentLoading] = (0, react_1.useState)(false); // State to track content loading
    ...
    const [selectedExample, setSelectedExample] = (0, react_1.useState)(PRESEED_CONTENT ? defaultExample : null);
    ...
    const handleSubmit = async () => {
        ...
        const isPreSeededExample = [defaultExample, ...examples].some((example) => example.url === inputValue);
        ...
        if (VALIDATE_INPUT_URL) {
            // Validate video URL
            const validationResult = await (0, youtube_1.validateYoutubeUrl)(inputValue);
    ...
    const handleContentLoadingStateChange = (isLoading) => {
        setContentLoading(isLoading);
    };
    const exampleGallery = (<ExampleGallery_1.default title={PRESEED_CONTENT ? 'More examples' : 'Examples'} onSelectExample={handleExampleSelect} selectedExample={selectedExample}/>);
    return (<>
      <main className="main-container">
        <div className="left-side">
          ...
          <div className="input-container">
            ...
            <input ref={inputRef} id="youtube-url" className="youtube-input" type="text" placeholder="https://www.youtube.com/watch?v=..." defaultValue={PRESEED_CONTENT ? defaultExample?.url : ''} disabled={urlValidating || contentLoading} // Disable input while validating or loading
     ...
     onChange={() => {
          ...
          <div className="button-container">
            <button onClick={handleSubmit} className="button-primary submit-button" disabled={urlValidating || contentLoading} // Disable button during validation or content loading
    >
              {urlValidating
            ? 'Validating URL...'
            : contentLoading
          ...
          <div className="video-container">
            {videoUrl ? (<iframe className="video-iframe" src={(0, youtube_1.getYoutubeEmbedUrl)(videoUrl)} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen></iframe>) : (<div className="video-placeholder">Video will appear here</div>)}
          </div>
        ...
        <div className="right-side">
          <div className="content-area">
            {videoUrl ? (<ContentContainer_1.default key={reloadCounter} contentBasis={videoUrl} onLoadingStateChange={handleContentLoadingStateChange} preSeededSpec={selectedExample?.spec} preSeededCode={selectedExample?.code} ref={contentContainerRef}/>) : (<div className="content-placeholder">
                <p>
                  {urlValidating
                ? 'Validating URL...'
