...
export default function AgentFlowVisualizer({
  ...
  const viewBox = useMemo(() => {
    ...
    // ⚡ Bolt: Replace spread operator mapped arrays with a single for loop to compute boundaries.
    // Expected impact: Prevents "Maximum call stack size exceeded" errors on large lists and improves performance by reducing O(N) passes from 4 to 1 and eliminating intermediate array allocations.
    let minX = Infinity;
    let minY = Infinity;
    let maxX = -Infinity;
    let maxY = -Infinity;
