...
const SECTIONS = [
    {
        ...
        subtitle: 'Full verbatim transcripts with speaker timestamps — in under 60 seconds. No manual editing required.',
        icon: '⚡',
    ...
    },
    {
        ...
        subtitle: 'Gemini 2.0 and GPT-4o go beyond transcription to surface what actually matters in any video.',
        icon: '🧠',
    ...
    },
    {
        ...
        subtitle: 'Every video becomes a knowledge base you can query in plain English. No more scrubbing through recordings.',
        icon: '💬',
    ...
    },
    {
        ...
        subtitle: 'Export structured intelligence to the tools your team already uses. One click, fully formatted.',
        icon: '📤',
    ...
    },
    {
        ...
        subtitle: 'Our experimental deploy pipeline watches a coding tutorial and produces a running application — fully automated.',
        icon: '🚀',
    ...
    },
    {
        ...
        subtitle: 'EventRelay is built around the Model Context Protocol. Extracted events become triggers for intelligent agents.',
        icon: '🔌',
...
function FeaturesPage() {
    ...
    return (<div className="min-h-screen text-white overflow-x-hidden">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-6 lg:px-12 py-4 border-b border-white/[0.05] bg-surface-950/80 backdrop-blur-xl">
        ...
        <div className="flex items-center gap-3">
          <link_1.default href="/features" className="hidden md:block text-sm text-white/80 hover:text-white transition px-3 py-2">
            Features
          </link_1.default>
          <link_1.default href="/pricing" className="hidden md:block text-sm text-white/50 hover:text-white transition px-3 py-2">
            Pricing
          </link_1.default>
          <link_1.default href="/dashboard" className="btn btn-secondary py-2 px-4 text-sm">
            Dashboard
          </link_1.default>
          <link_1.default href="/dashboard" className="btn btn-primary py-2 px-4 text-sm">
            Get started free →
          </link_1.default>
        </div>
      </nav>
      ...
      <section className="relative pt-32 pb-16 px-6 text-center max-w-4xl mx-auto">
        ...
        <div className="inline-block px-3 py-1 rounded-full bg-white/[0.04] border border-white/[0.08] text-xs text-white/40 font-semibold uppercase tracking-widest mb-6 animate-fade-in-up opacity-0" style={{ animationDelay: '0ms', animationFillMode: 'forwards' }}>
          Features
        </div>

        <h1 className="text-5xl md:text-6xl font-black leading-tight tracking-tight mb-5 animate-fade-in-up opacity-0" style={{ animationDelay: '80ms', animationFillMode: 'forwards' }}>
        ...
        <div className="flex flex-wrap items-center justify-center gap-4 animate-fade-in-up opacity-0" style={{ animationDelay: '240ms', animationFillMode: 'forwards' }}>
          <link_1.default href="/dashboard" className="btn btn-primary py-3.5 px-8 text-sm shadow-lg shadow-primary-500/30">
            Try it free →
          </link_1.default>
          <link_1.default href="/pricing" className="btn btn-secondary py-3.5 px-8 text-sm">
            View pricing
          </link_1.default>
        </div>

        {/* Quick feature pills */}
        <div className="flex flex-wrap items-center justify-center gap-3 mt-10 animate-fade-in-up opacity-0" style={{ animationDelay: '320ms', animationFillMode: 'forwards' }}>
          {SECTIONS.map((s) => (<a key={s.id} href={`#${s.id}`} onClick={() => setActiveSection(s.id)} className={(0, clsx_1.clsx)('px-3 py-1.5 rounded-full border text-xs font-medium transition-all', activeSection === s.id
                ? s.tagColor
      ...
      <div className="max-w-6xl mx-auto px-6 space-y-28 mb-28 pt-8">
        {SECTIONS.map((section, sectionIdx) => {
            ...
            return (<section key={section.id} id={section.id}>
              <div className={(0, clsx_1.clsx)('grid grid-cols-1 lg:grid-cols-2 gap-12 items-center')}>
                {/* Visual panel — swaps order on alternating rows */}
                <div className={(0, clsx_1.clsx)('animate-fade-in-up opacity-0', !isEven && 'lg:order-2')} style={{ animationDelay: `${sectionIdx * 50}ms`, animationFillMode: 'forwards' }}>
                  <div className={(0, clsx_1.clsx)('relative rounded-3xl p-10 border flex items-center justify-center overflow-hidden', `bg-gradient-to-br ${section.iconBg}`, section.iconBorder)} style={{ minHeight: '340px' }}>
                    {/* Decorative rings */}
                    ...
                    <div className="relative text-center">
                      ...
                      <div>
                        <div className={(0, clsx_1.clsx)('inline-flex items-center gap-2 px-4 py-2 rounded-full border text-xs font-semibold', section.tagColor)}>
                          {section.tag}
                          {section.isExperimental && (<span className="px-1.5 py-0.5 rounded-full bg-pink-500/20 border border-pink-500/30 text-pink-300 text-xs ml-1">
                              Experimental
                            </span>)}
                        </div>
                      </div>
                ...
                {/* Text panel */}
                <div className={(0, clsx_1.clsx)('animate-fade-in-up opacity-0', !isEven && 'lg:order-1')} style={{ animationDelay: `${sectionIdx * 50 + 80}ms`, animationFillMode: 'forwards' }}>
                  <div className={(0, clsx_1.clsx)('inline-flex items-center gap-2 px-3 py-1 rounded-full border text-xs font-semibold mb-4', section.tagColor)}>
                    {section.tag}
                    {section.isExperimental && (<span className="px-1.5 py-0.5 rounded-full bg-pink-500/20 border border-pink-500/30 text-pink-300 text-xs">
                        Experimental
                      </span>)}
                  </div>
                  ...
                  <ul className="space-y-5">
                    {section.bullets.map((b) => (<li key={b.title} className="flex gap-4">
      ...
      <section className="max-w-4xl mx-auto px-6 mb-24">
        ...
        ].map((stat, i) => (<div key={stat.label} className="text-center p-6 rounded-2xl bg-white/[0.03] border border-white/[0.06] animate-fade-in-up opacity-0" style={{ animationDelay: `${i * 80}ms`, animationFillMode: 'forwards' }}>
      ...
      <section className="max-w-5xl mx-auto px-6 mb-24">
        ...
        <div className="rounded-2xl border border-white/[0.08] overflow-hidden">
          ...
          {COMPARISON_ROWS.map((row, i) => (<div key={row.feature} className={(0, clsx_1.clsx)('grid grid-cols-5 border-b border-white/[0.04] last:border-0 items-center', i % 2 === 0 ? 'bg-transparent' : 'bg-white/[0.01]')}>
      ...
      <section className="max-w-5xl mx-auto px-6 mb-24">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {[
            ...
            },
            {
        ...
        ].map((t, i) => (<div key={t.name} className="p-6 rounded-2xl bg-white/[0.03] border border-white/[0.06] hover:border-white/[0.12] transition-all animate-fade-in-up opacity-0" style={{ animationDelay: `${i * 120}ms`, animationFillMode: 'forwards' }}>
              ...
              <div className="flex items-center gap-3">
                <div className={(0, clsx_1.clsx)('w-9 h-9 rounded-full flex items-center justify-center font-bold text-sm text-white', t.color)}>
                  {t.avatar}
                </div>
                <div>
      ...
      <section className="max-w-3xl mx-auto px-6 pb-24 text-center">
        <div className="p-12 rounded-3xl bg-gradient-to-br from-primary-500/15 via-primary-500/5 to-cyan-500/5 border border-primary-500/20">
          ...
          <div className="flex flex-wrap gap-4 justify-center">
            <link_1.default href="/dashboard" className="btn btn-primary py-4 px-10 text-base shadow-2xl shadow-primary-500/30">
              Start analyzing for free →
            </link_1.default>
            <link_1.default href="/pricing" className="btn btn-secondary py-4 px-8 text-base">
              View pricing
            </link_1.default>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-10">
            ...
            <div>
              ...
              <ul className="space-y-2.5 text-xs text-white/35">
                {[['Dashboard', '/dashboard'], ['Features', '/features'], ['Pricing', '/pricing'], ['API Docs', '/playground']].map(([label, href]) => (<li key={label}>
                    <link_1.default href={href} className="hover:text-white/60 transition">{label}</link_1.default>
                  </li>))}
              </ul>
            </div>
            <div>
              ...
              <ul className="space-y-2.5 text-xs text-white/35">
                {['Meeting Notes', 'Conference Talks', 'Tutorials', 'Product Demos', 'Podcasts'].map((u) => (<li key={u}><span className="cursor-default">{u}</span></li>))}
              </ul>
            </div>
            <div>
