...
const youtube_metadata_1 = require("@/lib/youtube-metadata");
...
async function POST(request) {
    try {
        ...
        if (!url && !audioUrl) {
            return server_1.NextResponse.json({ error: 'url or audioUrl is required' }, { status: 400 });
        }
        ...
        if (url) {
            try {
                metadata = await (0, youtube_metadata_1.fetchYouTubeMetadata)(url);
            }
            catch {
        ...
        if (url && !audioUrl && (0, gemini_client_1.hasGeminiKey)()) {
            try {
                const ai = (0, gemini_client_1.getGeminiClient)();
                const metadataContext = metadata ? (0, youtube_metadata_1.formatMetadataAsContext)(metadata) : '';
...
${metadataContext ? `\nKNOWN METADATA:\n${metadataContext}` : ''}`,
        ...
        if (audioUrl && process.env.OPENAI_API_KEY) {
            ...
            if (!audioResponse.ok) {
                return server_1.NextResponse.json({ error: `Failed to fetch audio: ${audioResponse.status}` }, { status: 400 });
            }
        ...
        const hasKeys = !!(process.env.OPENAI_API_KEY || (0, gemini_client_1.hasGeminiKey)());
        return server_1.NextResponse.json({
            success: false,
            error: hasKeys
