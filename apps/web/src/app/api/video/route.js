...
const cloudevents_1 = require("@/lib/cloudevents");
const gemini_video_analyzer_1 = require("@/lib/gemini-video-analyzer");
...
async function POST(request) {
    ...
    try {
        ...
        videoUrl = url;
        if (!url) {
            return server_1.NextResponse.json({ error: 'Video URL is required' }, { status: 400 });
        }
        await (0, cloudevents_1.publishEvent)(cloudevents_1.EventTypes.VIDEO_RECEIVED, { url }, url);
        ...
        if (BACKEND_AVAILABLE) {
            try {
                ...
                if (response.ok) {
                    ...
                    const insights = {
                        summary: summaryText,
                        actions: transcriptAction.task_board?.tasks?.map((t) => t.title) || [],
                    ...
                    await (0, cloudevents_1.publishEvent)(cloudevents_1.EventTypes.PIPELINE_COMPLETED, { strategy: 'backend', success: result.success, agents: result.orchestration_meta?.agents_used || [] }, url);
        ...
        if ((0, gemini_client_1.hasGeminiKey)()) {
            try {
                await (0, cloudevents_1.publishEvent)(cloudevents_1.EventTypes.TRANSCRIPT_STARTED, { url, strategy: 'gemini-agentic' }, url);
                const startTime = Date.now();
                const analysis = await (0, gemini_video_analyzer_1.analyzeVideoWithGemini)(url);
                const elapsed = Date.now() - startTime;
                await (0, cloudevents_1.publishEvent)(cloudevents_1.EventTypes.PIPELINE_COMPLETED, {
        ...
        try {
            await (0, cloudevents_1.publishEvent)(cloudevents_1.EventTypes.TRANSCRIPT_STARTED, { url, strategy: 'frontend-chain' }, url);
            ...
            if (transcribeResult.success && transcribeResult.transcript) {
                ...
                await (0, cloudevents_1.publishEvent)(cloudevents_1.EventTypes.TRANSCRIPT_COMPLETED, { source: transcriptSource, wordCount: transcript.split(/\s+/).length }, url);
            }
        }
        catch (e) {
        ...
        if (transcript) {
            try {
                await (0, cloudevents_1.publishEvent)(cloudevents_1.EventTypes.EXTRACTION_STARTED, { transcriptLength: transcript.length }, url);
                const baseUrl = getBaseUrl(request);
                ...
                if (extractResult.success && extractResult.data) {
                    ...
                    await (0, cloudevents_1.publishEvent)(cloudevents_1.EventTypes.EXTRACTION_COMPLETED, { events: extraction.events?.length || 0, actions: extraction.actions?.length || 0 }, url);
                }
            }
            catch (e) {
        ...
        await (0, cloudevents_1.publishEvent)(hasResults ? cloudevents_1.EventTypes.PIPELINE_COMPLETED : cloudevents_1.EventTypes.PIPELINE_FAILED, { strategy: 'frontend-chain', success: hasResults, transcriptSource }, url);
        return server_1.NextResponse.json({
            id: `vid_${Date.now().toString(36)}`,
        ...
        await (0, cloudevents_1.publishEvent)(cloudevents_1.EventTypes.PIPELINE_FAILED, { error: String(error) }, videoUrl).catch(() => { });
        return server_1.NextResponse.json({ error: 'Failed to analyze video', details: String(error) }, { status: 500 });
    }
}
async function GET() {
