...
async function POST(request) {
    ...
    try {
        ...
        await (0, cloudevents_1.publishEvent)(cloudevents_1.EventTypes.VIDEO_RECEIVED, { url, pipeline: 'end-to-end' }, url);
        ...
        return server_1.NextResponse.json({ error: 'No pipeline available. Configure BACKEND_URL for full pipeline or GEMINI_API_KEY for analysis only.' }, { status: 503 });
    }
    catch (error) {
        console.error('Pipeline error:', error);
        await (0, cloudevents_1.publishEvent)(cloudevents_1.EventTypes.PIPELINE_FAILED, { error: String(error) }, videoUrl).catch(() => { });
        return server_1.NextResponse.json({ error: 'Pipeline failed', details: String(error) }, { status: 500 });
    }
}
async function GET() {
    return server_1.NextResponse.json({
        ...
        gemini_available: (0, gemini_client_1.hasGeminiKey)(),
        endpoints: {
