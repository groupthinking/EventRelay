...
exports.default = APIPlaygroundPage;
...
function CodeEditor({ value, onChange, language = 'json' }) {
    return (<div className="relative">
      ...
      <textarea value={value} onChange={(e) => onChange(e.target.value)} className="w-full h-64 p-4 bg-slate-900 rounded-xl border border-white/10 font-mono text-sm text-white/80 resize-none focus:outline-none focus:border-violet-500/50" spellCheck={false}/>
    </div>);
}
// Response viewer
function ResponseViewer({ response }) {
    if (response.status === 'loading') {
        return (<div className="flex items-center justify-center h-64 bg-slate-900 rounded-xl border border-white/10">
    ...
    return (<div className="relative">
      <div className={`absolute top-2 right-2 px-2 py-1 rounded text-xs ${response.status === 'success' ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>
...
// Endpoint selector
function EndpointCard({ method, endpoint, description, isSelected, onClick }) {
    const methodColors = {
    ...
    return (<button onClick={onClick} className={`w-full text-left p-4 rounded-xl border transition-all ${isSelected
...
function APIPlaygroundPage() {
    const [selectedEndpoint, setSelectedEndpoint] = (0, react_1.useState)(0);
    const [requestBody, setRequestBody] = (0, react_1.useState)(`{
  "video_url": "https://youtube.com/watch?v=example",
  "task": "Summarize this video and extract key insights"
}`);
    const [response, setResponse] = (0, react_1.useState)({ status: null, data: null });
    ...
    return (<div className="min-h-screen bg-slate-950 text-white">
      ...
      <nav className="relative z-50 flex items-center justify-between px-6 py-4 border-b border-white/5">
        ...
        <div className="flex items-center gap-4">
          <link_1.default href={`${BASE_URL}/docs`} target="_blank" className="px-4 py-2 rounded-lg bg-white/5 border border-white/10 text-sm hover:bg-white/10 transition">
            Full API Docs →
          </link_1.default>
        </div>
      </nav>
      ...
      <div className="relative z-10 max-w-7xl mx-auto p-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          <div className="space-y-3">
            ...
            {endpoints.map((ep, i) => (<EndpointCard key={i} method={ep.method} endpoint={ep.endpoint} description={ep.description} isSelected={selectedEndpoint === i} onClick={() => setSelectedEndpoint(i)}/>))}
          </div>
          ...
          <div className="lg:col-span-3 space-y-6">
            ...
            <div className="flex items-center gap-3 p-4 bg-white/5 rounded-xl border border-white/10">
              <span className={`px-3 py-1 rounded-lg text-sm font-bold ${currentEndpoint.method === 'GET' ? 'bg-green-500/20 text-green-400' :
            currentEndpoint.method === 'POST' ? 'bg-blue-500/20 text-blue-400' :
              ...
              <button onClick={handleSendRequest} className="px-6 py-2 rounded-lg bg-gradient-to-r from-violet-600 to-purple-600 font-medium hover:opacity-90 transition">
                Send Request
              </button>
            </div>
            ...
            <div>
              ...
              <CodeEditor value={requestBody} onChange={setRequestBody}/>
            </div>
