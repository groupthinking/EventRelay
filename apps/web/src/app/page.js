...
function Home() {
    ...
    return (<div className="min-h-screen text-white">
      ...
      <nav className="flex items-center justify-between px-6 lg:px-12 py-4 border-b border-white/[0.05]">
        ...
        <div className="flex items-center gap-3">
          <link_1.default href="/dashboard" className="btn btn-secondary py-2 text-sm">
            Dashboard
          </link_1.default>
          <link_1.default href="/playground" className="btn btn-ghost py-2 text-sm text-white/50">
            API
          </link_1.default>
        </div>
      </nav>
      ...
      <div className="max-w-3xl mx-auto px-6 pt-20 pb-12 text-center">
        ...
        {/* Input */}
        <form onSubmit={(e) => { e.preventDefault(); handleProcess(); }} className="max-w-2xl mx-auto">
          <div className="flex gap-3 p-2 rounded-2xl bg-white/[0.04] border border-white/[0.08] focus-within:border-primary-500/40 focus-within:shadow-lg focus-within:shadow-primary-500/5 transition-all">
            <input type="text" value={videoUrl} onChange={(e) => setVideoUrl(e.target.value)} placeholder="https://youtube.com/watch?v=..." className="flex-1 px-4 py-3 bg-transparent text-white placeholder:text-white/30 focus:outline-none text-sm"/>
            <button type="submit" disabled={!videoUrl.trim()} className="btn btn-primary py-3 px-8 disabled:opacity-30 disabled:cursor-not-allowed">
              Process Video
            </button>
          </div>
        </form>
        ...
        <div className="flex flex-wrap items-center justify-center gap-3 mt-5">
          ...
          {EXAMPLES.map(({ url, label }) => (<button key={url} onClick={() => { setVideoUrl(url); }} className="text-xs px-3 py-1.5 rounded-lg bg-white/[0.04] border border-white/[0.06] text-primary-400/80 hover:text-primary-400 hover:border-primary-500/30 transition">
              {label}
            </button>))}
        </div>
      </div>
      ...
      <div className="max-w-3xl mx-auto px-6 pb-20">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {STEPS.map((step, i) => (<div key={step.title} className={(0, clsx_1.clsx)('relative p-6 rounded-2xl bg-white/[0.03] border border-white/[0.06]', 'animate-fade-in-up opacity-0')} style={{ animationDelay: `${i * 150}ms`, animationFillMode: 'forwards' }}>
      ...
      <div className="border-t border-white/[0.06] py-6">
        <div className="max-w-3xl mx-auto px-6 flex items-center justify-between text-xs text-white/25">
          ...
          <div className="flex items-center gap-4">
            <link_1.default href="/playground" className="hover:text-white/50 transition">API</link_1.default>
