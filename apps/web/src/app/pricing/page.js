...
function FaqItem({ q, a }) {
    ...
    return (<div className="border border-white/[0.08] rounded-2xl overflow-hidden transition-all" style={{ background: open ? 'rgba(255,255,255,0.03)' : 'transparent' }}>
      <button onClick={() => setOpen((o) => !o)} className="w-full flex items-center justify-between px-6 py-5 text-left gap-4 hover:bg-white/[0.03] transition-colors">
        <span className="font-semibold text-white/90">{q}</span>
        <span className={(0, clsx_1.clsx)('w-6 h-6 flex-shrink-0 rounded-full bg-white/[0.05] flex items-center justify-center text-white/50 transition-transform duration-300', open && 'rotate-45')}>
          +
        </span>
      </button>
      {open && (<div className="px-6 pb-5">
          <p className="text-white/55 leading-relaxed text-sm">{a}</p>
        </div>)}
    </div>);
}
function CheckItem({ text, color = 'text-green-400' }) {
    return (<li className="flex items-start gap-2.5 text-sm text-white/65">
      <span className={(0, clsx_1.clsx)('mt-0.5 flex-shrink-0 font-bold', color)}>✓</span>
      <span>{text}</span>
    </li>);
}
// ─── Main Page ─────────────────────────────────────────────────────────────────
    ...
    return (<div className="min-h-screen text-white overflow-x-hidden">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-6 lg:px-12 py-4 border-b border-white/[0.05] bg-surface-950/80 backdrop-blur-xl">
        ...
        <div className="flex items-center gap-3">
          <link_1.default href="/features" className="hidden md:block text-sm text-white/50 hover:text-white transition px-3 py-2">
            Features
          </link_1.default>
          <link_1.default href="/pricing" className="hidden md:block text-sm text-white/80 hover:text-white transition px-3 py-2">
            Pricing
          </link_1.default>
      ...
      <section className="relative pt-32 pb-16 px-6 text-center max-w-4xl mx-auto">
        ...
        <p className="text-lg text-white/50 max-w-xl mx-auto mb-10 leading-relaxed animate-fade-in-up opacity-0" style={{ animationDelay: '160ms', animationFillMode: 'forwards' }}>
          Start for free. Upgrade when you need more power.
          No hidden fees, no surprises, cancel any time.
        </p>

        {/* Billing toggle */}
        <div className="inline-flex items-center gap-3 p-1.5 rounded-2xl bg-white/[0.04] border border-white/[0.08] animate-fade-in-up opacity-0" style={{ animationDelay: '240ms', animationFillMode: 'forwards' }}>
          <button onClick={() => setAnnual(false)} className={(0, clsx_1.clsx)('px-5 py-2 rounded-xl text-sm font-semibold transition-all duration-200', !annual
            ? 'bg-white/[0.1] text-white shadow-sm'
            : 'text-white/40 hover:text-white/60')}>
            Monthly
          </button>
          <button onClick={() => setAnnual(true)} className={(0, clsx_1.clsx)('px-5 py-2 rounded-xl text-sm font-semibold transition-all duration-200 flex items-center gap-2', annual
            ? 'bg-white/[0.1] text-white shadow-sm'
            : 'text-white/40 hover:text-white/60')}>
            Annual
      ...
      <section className="max-w-6xl mx-auto px-6 mb-20">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-stretch">
          ...
          {/* Free */}
          <div className="relative p-8 rounded-3xl bg-white/[0.02] border border-white/[0.08] flex flex-col hover:border-white/[0.14] transition-all duration-300 animate-fade-in-up opacity-0" style={{ animationDelay: '100ms', animationFillMode: 'forwards' }}>
            <div>
            ...
            <ul className="space-y-3 mb-8 flex-1">
              {FREE_FEATURES.map((f) => (<CheckItem key={f} text={f} color="text-green-400"/>))}
            </ul>

            <link_1.default href="/dashboard" className="btn btn-secondary py-3.5 w-full text-sm text-center">
              Get started free
            </link_1.default>
          </div>

          {/* Pro */}
          <div className="relative p-8 rounded-3xl border border-primary-500/40 flex flex-col shadow-2xl shadow-primary-500/10 animate-fade-in-up opacity-0" style={{
            ...
            <div>
              ...
              {annual && (<div className="inline-flex items-center gap-1.5 mb-2 px-2.5 py-1 rounded-full bg-green-500/10 border border-green-500/20">
            ...
            <ul className="space-y-3 mb-8 flex-1">
              {PRO_FEATURES.map((f) => (<CheckItem key={f} text={f} color="text-primary-400"/>))}
            </ul>

            <link_1.default href="/dashboard" className="btn btn-primary py-3.5 w-full text-sm text-center shadow-lg shadow-primary-500/30">
          ...
          {/* Enterprise */}
          <div className="relative p-8 rounded-3xl bg-white/[0.02] border border-white/[0.08] flex flex-col hover:border-cyan-500/20 transition-all duration-300 animate-fade-in-up opacity-0" style={{ animationDelay: '300ms', animationFillMode: 'forwards' }}>
            <div>
            ...
            <ul className="space-y-3 mb-8 flex-1">
              {ENTERPRISE_FEATURES.map((f) => (<CheckItem key={f} text={f} color="text-cyan-400"/>))}
            </ul>

            <a href="mailto:enterprise@eventrelay.ai" className="btn btn-secondary py-3.5 w-full text-sm text-center border-cyan-500/20 hover:border-cyan-500/40">
              Contact sales →
            </a>
          </div>
        </div>
      ...
      <section className="max-w-4xl mx-auto px-6 mb-24">
        ...
        ].map((row, i) => (<div key={row.label} className={(0, clsx_1.clsx)('grid grid-cols-4 border-b border-white/[0.04] last:border-0', i % 2 === 0 ? 'bg-transparent' : 'bg-white/[0.01]')}>
              <div className="p-4 text-sm text-white/60">{row.label}</div>
              <div className={(0, clsx_1.clsx)('p-4 text-sm text-center', row.free === '—' ? 'text-white/20' : 'text-white/60')}>{row.free}</div>
              <div className={(0, clsx_1.clsx)('p-4 text-sm text-center font-medium', row.pro === '—' ? 'text-white/20' : 'text-primary-400')}>{row.pro}</div>
              <div className={(0, clsx_1.clsx)('p-4 text-sm text-center font-medium', row.ent === '—' ? 'text-white/20' : 'text-cyan-400')}>{row.ent}</div>
            </div>))}
        </div>
      </section>
      ...
      <section className="max-w-3xl mx-auto px-6 mb-24">
        <div className="p-10 rounded-3xl bg-white/[0.02] border border-white/[0.08] text-center">
          ...
          <a href="mailto:hello@eventrelay.ai" className="btn btn-secondary py-3 px-6 text-sm inline-flex">
            Email us →
          </a>
        </div>
      </section>
      ...
      <section className="max-w-3xl mx-auto px-6 pb-24 text-center">
        <div className="p-12 rounded-3xl bg-gradient-to-br from-primary-500/15 via-primary-500/5 to-cyan-500/5 border border-primary-500/20">
          ...
          <link_1.default href="/dashboard" className="btn btn-primary py-4 px-10 text-base shadow-2xl shadow-primary-500/30 inline-block">
            Get started free →
          </link_1.default>
