...
Includes Vision-Reasoning Stack integration
"""
...
class MCPStateCoordinator:
    def __init__(
        self, db_path: str = "/Users/garvey/mcp-ecosystem/shared-state/mcp_state.db"
    ):
    ...
    def update_server_status(
        self, server_id: str, status: str, current_task: str = None
    ):
        ...
        if server_id in self.servers:
            ...
            asyncio.create_task(
                self._broadcast_server_update(server_id, "status_updated")
            )
    ...
    async def update_action_status(
        self, action_id: str, status: str, results: Dict[str, Any] = None
    ):
    ...
    async def _check_dependent_actions(self, completed_action_id: str):
        ...
        for action_id, action in self.actions.items():
            if (
                action.dependencies
                and completed_action_id in action.dependencies
    ...
    def get_pending_actions(self, server_id: str = None) -> List[SharedAction]:
        ...
        actions = [
            ...
            if action.status in ["pending", "ready"]
        ]
        if server_id:
    ...
    async def _broadcast_server_update(self, server_id: str, event_type: str):
        ...
        message = {
            ...
            "server_id": server_id,
            "server_state": (
                asdict(self.servers[server_id]) if server_id in self.servers else None
            ),
            "timestamp": time.time(),
        }
    ...
    async def _broadcast_action_update(self, action_id: str, event_type: str):
        ...
        message = {
            ...
            "action_id": action_id,
            "action": (
                asdict(self.actions[action_id]) if action_id in self.actions else None
            ),
            "timestamp": time.time(),
        }
    ...
    async def _handle_websocket_message(self, websocket, data: Dict[str, Any]):
        ...
        if msg_type == "register_server":
            ...
            await websocket.send(
                json.dumps({"status": "registered", "server_id": server_id})
            )
            ...
            action_id = await self.submit_action(
                server_id, action_type, payload, dependencies
            )
            await websocket.send(
                json.dumps({"status": "submitted", "action_id": action_id})
            )
            ...
            servers = {
                sid: asdict(server)
                for sid, server in self.get_available_servers().items()
            }
            ...
            logger.info(
                f"Received tool_call_request for {target_server_id}/{tool_name} (ID: {request_id})"
            )

            if (
                target_server_id not in self.servers
                or not self.servers[target_server_id].websocket.open
            ):
                error_message = (
                    f"Target server {target_server_id} not found or not connected."
                )
            ...
            try:
                ...
                logger.info(
                    f"Tool call response from {target_server_id}: {response_from_target}"
                )
                ...
                error_message = (
                    f"Connection to target server {target_server_id} closed: {e}"
                )
                logger.error(error_message)
                await websocket.send(
                    json.dumps(
        ...
        # ============================================
        # Vision-Reasoning Stack Message Handlers
        # ============================================

        elif msg_type == "vision_process_frame":
            # Process single frame for OCR/label detection (clapperboard workflow)
            frame_data_b64 = data.get("frame_data")  # Base64 encoded image
            detect_labels = data.get("detect_labels", True)
            extract_ocr = data.get("extract_ocr", True)
            ...
            try:
                ...
                # Check cache first
                cache_key = f"frame:{hash(frame_data_b64)}"
                if use_cache:
                ...
                # Process frame
                result = await self.vision_processor.process_frame(
                    frame_data, detect_labels, extract_ocr
                )
