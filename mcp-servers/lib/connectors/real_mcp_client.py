...
class MCPClient:
    ...
    def __init__(self, server_path: Optional[str] = None):
        self.server_path = server_path or str(
            Path(__file__).parent.parent.parent / "python-suite" / "youtube_uvai_mcp.py"
        )
    ...
    async def connect(self, max_connection_attempts: int = 3) -> bool:
        ...
        for attempt in range(max_connection_attempts):
            try:
                logger.info(
                    f"Starting MCP server (attempt {attempt + 1}): {self.server_path}"
                )
                ...
                if result and "serverInfo" in result:
                    ...
                    logger.info(
                        f"Connected to MCP server: {self.server_info.get('name')}"
                    )
    ...
    async def execute_tool(
        ...
        try:
            ...
            client = await asyncio.wait_for(
                self.available_clients.get(), timeout=timeout
            )
        except asyncio.TimeoutError:
            logger.warning(
                f"Timeout waiting for MCP client after {timeout}s for tool: {tool_name}"
            )
            return {
                "tool": tool_name,
...
async def execute_mcp_tool(
    tool_name: str, arguments: Dict[str, Any], max_retries: int = 3
) -> Dict[str, Any]:
