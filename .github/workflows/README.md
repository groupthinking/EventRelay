...
| PR Iteration Loop | `pr-iteration-loop.md` / `.lock.yml` | issue/PR labeled `run-pr-iteration`; issue comment command `/run-pr-iteration-loop`; weekdays; push to `main` | Deterministic single-fingerprint loop with early dedupe/no-op, one canonical issue + draft PR mutation path on `pr-iteration/*` branches only, and human-only merges |
