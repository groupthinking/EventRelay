...
- **Removed** `verify-litert-mcp.yml` and `vision-reasoning.yml` — both only
  exercised the `mcp-servers/` tree (`litert-mcp/server.py` and
  `shared-state/*` respectively). The cleanup pass removed `mcp-servers/`
  entirely, so these path-filtered workflows triggered on the deletion and then
  failed at runtime trying to `python3 mcp-servers/...` / `pip install -r
  mcp-servers/requirements.txt` against paths that no longer exist.
