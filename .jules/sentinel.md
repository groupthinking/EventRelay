...
## 2026-07-17 - Fix SQL Injection in Innovation Statistics
**Vulnerability:** SQL Injection via direct f-string string interpolation of variable `time_filter` in SQLite `conn.execute()` commands.
**Learning:** Even internal reporting / analytics dashboards using dynamically generated date boundaries can introduce SQL injection if the dynamic string component is not properly separated from the core SQL using parameterized execution.
**Prevention:** Always use parameterized SQL statements (e.g. `?` placeholders) and securely format variables (like dates) in Python prior to binding them to the driver parameters. Avoid creating direct SQL strings using string interpolation.
