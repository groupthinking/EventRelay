...
@pytest.mark.asyncio
async def test_run_repository_research_harness_executes_skills_in_order(
    registry: SkillRegistry,
) -> None:
    ...
    assert result["skill_outputs"]["canonical-architecture-truth"]["canonical_vs_legacy_map"][
        "public_product_name"
    ] == "UVAI"
    assert result["output"]["workflow health report"]["failing_workflows"] == [
        "verification.yml"
    ]


@pytest.mark.asyncio
async def test_run_repository_research_harness_fails_closed_on_skill_error(
    registry: SkillRegistry,
) -> None:
    result = await registry.run_repository_research_harness(
        skill_inputs={
    ...
    assert result["status"] == "error"
    assert result["failed_skill"] == "canonical-architecture-truth"
    assert "evidence_sources" in (result["error"] or "")
