...
class TestFallbackImports:
    def test_log_error_fallback_is_callable(self):
    ...
    def test_get_logger_fallback_returns_logger(self):
        import youtube_extension.backend.services.health_monitoring_service as _mod
...
def _make_full_psutil(*, bytes_sent=1000, bytes_recv=2000, packets_sent=10,
                       packets_recv=20, disk_percent_used=40):
...
class TestSystemResourceCheckerDiskNetwork:
    async def test_disk_metric_included(self, monkeypatch):
        import youtube_extension.backend.services.health_monitoring_service as _mod
    ...
    async def test_disk_metric_value(self, monkeypatch):
        import youtube_extension.backend.services.health_monitoring_service as _mod
    ...
    async def test_disk_critical_when_high(self, monkeypatch):
        import youtube_extension.backend.services.health_monitoring_service as _mod
    ...
    async def test_disk_warning_when_moderate(self, monkeypatch):
        import youtube_extension.backend.services.health_monitoring_service as _mod
    ...
    async def test_network_metric_appears_on_second_call(self, monkeypatch):
    ...
    async def test_network_metric_has_send_recv_metadata(self, monkeypatch):
        import youtube_extension.backend.services.health_monitoring_service as _mod
        monkeypatch.setattr(_mod, 'psutil', _make_full_psutil())
        src = SystemResourceChecker()
    ...
    async def test_network_metric_status_is_healthy(self, monkeypatch):
        import youtube_extension.backend.services.health_monitoring_service as _mod
        monkeypatch.setattr(_mod, 'psutil', _make_full_psutil())
        src = SystemResourceChecker()
        await src.check_health()
    ...
    async def test_recovery_actions_present_on_critical(self, monkeypatch):
    ...
    async def test_exception_path_returns_failed(self, monkeypatch):
...
class TestDetermineStatusBoundary:
    ...
    def test_just_below_warning_is_healthy(self):
        src = SystemResourceChecker()
...
class TestDatabaseCheckerCheckHealth:
    async def test_returns_component_health(self):
        dc = DatabaseChecker("sqlite:///:memory:")
        result = await dc.check_health()
        assert isinstance(result, ComponentHealth)

    async def test_status_is_health_status(self):
        dc = DatabaseChecker("sqlite:///:memory:")
        result = await dc.check_health()
        assert isinstance(result.status, HealthStatus)

    async def test_has_connection_latency_metric(self):
        dc = DatabaseChecker("sqlite:///:memory:")
        result = await dc.check_health()
        names = [m.name for m in result.metrics]
        assert "connection_latency" in names

    async def test_has_query_performance_metric(self):
    ...
    async def test_critical_when_connection_fails(self, monkeypatch):
        dc = DatabaseChecker("sqlite:///:memory:")
    ...
    async def test_no_query_metric_when_connection_fails(self, monkeypatch):
        dc = DatabaseChecker("sqlite:///:memory:")
    ...
    async def test_recovery_actions_on_critical(self, monkeypatch):
        dc = DatabaseChecker("sqlite:///:memory:")
    ...
    async def test_high_latency_yields_warning_or_critical(self, monkeypatch):
        """Simulate a slow connection (>100 ms) via a slow _test_connection."""
        import asyncio as _asyncio
        dc = DatabaseChecker("sqlite:///:memory:")

        async def _slow():
            await _asyncio.sleep(0.0)  # just returns True quickly in tests
            return True
...
class TestExternalServiceCheckerCheckHealth:
    async def test_healthy_response(self, monkeypatch):
    ...
    async def test_500_response_is_critical(self, monkeypatch):
        esc = ExternalServiceChecker("test_svc", "http://example.com")
    ...
    async def test_404_response_is_warning(self, monkeypatch):
        esc = ExternalServiceChecker("test_svc", "http://example.com")
    ...
    async def test_connection_failed_zero_status(self, monkeypatch):
        esc = ExternalServiceChecker("test_svc", "http://example.com")
    ...
    async def test_slow_response_over_5s_is_critical(self, monkeypatch):
        esc = ExternalServiceChecker("test_svc", "http://example.com")
    ...
    async def test_slow_response_over_1s_is_warning(self, monkeypatch):
        esc = ExternalServiceChecker("test_svc", "http://example.com")
    ...
    async def test_response_metric_included(self, monkeypatch):
        esc = ExternalServiceChecker("test_svc", "http://example.com")
    ...
    def test_determine_http_status_zero_code_critical(self):
        esc = ExternalServiceChecker("svc", "http://example.com")
        assert esc._determine_http_status(0, 100.0) == HealthStatus.CRITICAL

    def test_determine_http_status_400_warning(self):
        esc = ExternalServiceChecker("svc", "http://example.com")
        assert esc._determine_http_status(400, 100.0) == HealthStatus.WARNING

    def test_determine_http_status_over_5000ms_warning(self):
        esc = ExternalServiceChecker("svc", "http://example.com")
        assert esc._determine_http_status(200, 6000.0) == HealthStatus.WARNING

    def test_determine_http_status_over_1000ms_warning(self):
        esc = ExternalServiceChecker("svc", "http://example.com")
        assert esc._determine_http_status(200, 2000.0) == HealthStatus.WARNING
...
class TestApplicationCheckerCheckHealth:
    ...
    async def test_has_error_rate_metric(self, monkeypatch):
        import youtube_extension.backend.services.health_monitoring_service as _mod
        monkeypatch.setattr(_mod, 'psutil', _make_full_psutil())
        ac = ApplicationChecker()
        result = await ac.check_health()
    ...
    async def test_has_avg_response_time_metric(self, monkeypatch):
        import youtube_extension.backend.services.health_monitoring_service as _mod
        monkeypatch.setattr(_mod, 'psutil', _make_full_psutil())
        ac = ApplicationChecker()
        result = await ac.check_health()
    ...
    async def test_has_process_memory_metric(self, monkeypatch):
        import youtube_extension.backend.services.health_monitoring_service as _mod
        monkeypatch.setattr(_mod, 'psutil', _make_full_psutil())
        ac = ApplicationChecker()
        result = await ac.check_health()
    ...
    async def test_status_healthy_with_no_errors(self, monkeypatch):
        import youtube_extension.backend.services.health_monitoring_service as _mod
        monkeypatch.setattr(_mod, 'psutil', _make_full_psutil())
        ac = ApplicationChecker()
        result = await ac.check_health()
    ...
    async def test_warning_recovery_actions_present(self, monkeypatch):
    ...
    async def test_psutil_process_exception_skipped(self, monkeypatch):
...
class TestHealthMonitoringServiceUtilityMethods:
    def test_get_current_health_none_when_no_history(self):
        svc = HealthMonitoringService(config=_MINIMAL_CONFIG)
        assert svc.get_current_health() is None

    def test_get_current_health_returns_last(self):
        svc = HealthMonitoringService(config=_MINIMAL_CONFIG)
        sh = SystemHealth(
    ...
    def test_get_health_history_empty_by_default(self):
        svc = HealthMonitoringService(config=_MINIMAL_CONFIG)
        assert svc.get_health_history() == []

    def test_get_health_history_filters_by_time(self):
    ...
    def test_record_error_no_op_without_app_checker(self):
        svc = HealthMonitoringService(config=_MINIMAL_CONFIG)
        svc.application_checker = None
        svc.record_error()  # should not raise

    def test_record_request_delegates_to_app_checker(self):
    ...
    def test_record_error_delegates_to_app_checker(self):
        config = dict(_MINIMAL_CONFIG)
        config["enable_application"] = True
        svc = HealthMonitoringService(config=config)
...
class TestPerformHealthCheck:
    async def test_perform_health_check_empty_checkers(self):
    ...
    async def test_perform_health_check_stores_history(self):
        svc = HealthMonitoringService(config=_MINIMAL_CONFIG)
        await svc.perform_health_check()
        assert len(svc.health_history) == 1

    async def test_perform_health_check_history_limit(self):
    ...
    async def test_perform_health_check_handles_checker_exception(self):
...
class TestCalculateOverallHealth:
    def test_empty_components_returns_failed(self):
        svc = HealthMonitoringService(config=_MINIMAL_CONFIG)
    ...
    def test_critical_component_degrades_score(self):
        svc = HealthMonitoringService(config=_MINIMAL_CONFIG)
        comp = ComponentHealth(
            name="db",
...
class TestGetActiveAlerts:
    def test_no_alerts_for_healthy_components(self):
        svc = HealthMonitoringService(config=_MINIMAL_CONFIG)
        comp = ComponentHealth(
            name="db",
            component_type=ComponentType.DATABASE,
    ...
    def test_alert_for_critical_component(self):
        svc = HealthMonitoringService(config=_MINIMAL_CONFIG)
        comp = ComponentHealth(
            name="db",
            component_type=ComponentType.DATABASE,
    ...
    def test_alert_for_warning_component(self):
        svc = HealthMonitoringService(config=_MINIMAL_CONFIG)
        comp = ComponentHealth(
            name="svc",
    ...
    def test_multiple_components_multiple_alerts(self):
        svc = HealthMonitoringService(config=_MINIMAL_CONFIG)
        comps = [
            ComponentHealth(
                name=f"c{i}",
...
class TestGetRecentIncidents:
    ...
    def test_single_history_returns_empty(self):
        svc = HealthMonitoringService(config=_MINIMAL_CONFIG)
        import time as _time
...
class TestGenerateRecommendations:
    def test_empty_components_no_recommendations(self):
        svc = HealthMonitoringService(config=_MINIMAL_CONFIG)
        recs = svc._generate_recommendations([])
        assert recs == []

    def test_critical_component_generates_urgent_recs(self):
        svc = HealthMonitoringService(config=_MINIMAL_CONFIG)
        comp = ComponentHealth(
            name="db",
            component_type=ComponentType.DATABASE,
    ...
    def test_warning_component_generates_monitor_recs(self):
        svc = HealthMonitoringService(config=_MINIMAL_CONFIG)
        comp = ComponentHealth(
            name="svc",
            component_type=ComponentType.EXTERNAL_SERVICE,
    ...
    def test_multiple_critical_adds_emergency_message(self):
        svc = HealthMonitoringService(config=_MINIMAL_CONFIG)
        comps = [
            ComponentHealth(
                name=f"c{i}",
...
class TestGetComponentMetrics:
    ...
    def test_returns_metrics_for_known_component(self):
        import time as _time
        svc = HealthMonitoringService(config=_MINIMAL_CONFIG)
        metric = HealthMetric(
            name="cpu_usage",
    ...
    def test_old_history_excluded_from_metrics(self):
        svc = HealthMonitoringService(config=_MINIMAL_CONFIG)
        metric = HealthMetric(
            name="cpu_usage",
            value=30.0,
...
class TestGetHealthMonitoringServiceSingleton:
    ...
    def test_returns_health_monitoring_service_instance(self, monkeypatch):
        import youtube_extension.backend.services.health_monitoring_service as _mod
        monkeypatch.setattr(_mod, '_health_service', None)
...
class TestStopMonitoring:
    async def test_stop_monitoring_sets_is_running_false(self):
        svc = HealthMonitoringService(config=_MINIMAL_CONFIG)
        svc.is_running = True
        await svc.stop_monitoring()
        assert svc.is_running is False
...
class TestStartMonitoring:
    async def test_start_monitoring_sets_is_running(self):
        ...
        async def _run_then_stop():
            ...
            task = _asyncio.create_task(svc.start_monitoring())
            await _asyncio.sleep(0)  # yield to let start_monitoring set flag
    ...
    async def test_start_monitoring_no_op_if_already_running(self):
        svc = HealthMonitoringService(config=_MINIMAL_CONFIG)
        svc.is_running = True
        ...
        async def _call_with_timeout():
            try:
                await _asyncio.wait_for(svc.start_monitoring(), timeout=0.05)
...
class TestInitializeCheckers:
    ...
    def test_application_checker_added(self):
        config = dict(_MINIMAL_CONFIG)
        config["enable_application"] = True
        svc = HealthMonitoringService(config=config)
    ...
    def test_application_checker_none_when_disabled(self):
        svc = HealthMonitoringService(config=_MINIMAL_CONFIG)
...
class TestGetBasicHealthStatusExtra:
    ...
    def test_websocket_unavailable_when_none(self):
        svc = HealthMonitoringService(config=_MINIMAL_CONFIG)
        result = svc.get_basic_health_status(None, None)
        assert result["components"]["websocket"] == "unavailable"

    def test_video_processor_available_when_has_attr(self):
        svc = HealthMonitoringService(config=_MINIMAL_CONFIG)
