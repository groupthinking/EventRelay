...
class TestReportingRoutes:
    ...
    def test_generate_dashboard_url_service_error(self):
        """Service error propagates as a sanitized HTTP 500.

        Regression guard for information disclosure: the response must carry
        the generic message and must NOT leak the raw exception text.
        """
        mock_service = MagicMock()
        mock_service.get_tenant_dashboard_url = MagicMock(
