...
class TestUnifiedAISDK:
    ...
    async def test_unified_request_unsupported_provider_returns_failure(self):
        # ModelProvider.GROK has no direct client in this SDK path; the contract
        # is a structured failure (or routing to MCPBridge), not an unhandled ValueError.
        sdk = UnifiedAISDK({"retry_attempts": 1})
        sdk._grok_client = None
