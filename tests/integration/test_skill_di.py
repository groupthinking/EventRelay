...
# Mock missing scoring_engine to allow importing the coordinator
mock_se = types.ModuleType("youtube_extension.processors.scoring_engine")
mock_se.ScoringEngine = MagicMock
sys.modules["youtube_extension.processors.scoring_engine"] = mock_se
...
async def test_invoke_skill_with_di():
    ...
    instance.execute = AsyncMock(return_value=SkillResult(status="success", output={"test": "ok"}))
