...
| Google sign-in retry | [Login task](https://app.notion.com/p/3dd3c2339c0481a2a35dce563f281ab1?pvs=204) and [PR #1984](https://github.com/groupthinking/EventRelay/pull/1984) | A failed or non-navigating Google sign-in initiation returns the button to a retryable state without exposing failure details. |
...
> **Control boundary:** A local test, a pull request, or a rendered checkout control does not prove a production deployment, a paid subscription, an OAuth completion, or an Origin G.A.T.E. authorization.
...
| Responsive Home experience | In one terminal run `npm --workspace=eventrelay-web run dev`. With that page open locally, use responsive browser tools to inspect the Home page at 360 px and 1280 px widths. | At 360 px and 1280 px, the heading, form, pricing summary, and checkout surface are visible with no horizontal scrolling. |
...
|---|---|---|
| Origin G.A.T.E. and sandbox origin controls | Confirm `redis-server` or `redis6-server` is installed with `command -v redis-server || command -v redis6-server`, then run `npm --workspace=eventrelay-web run test:gate`. This opt-in suite sets `ORIGIN_GATE_REDIS_TESTS=1` and starts an isolated local Redis socket for the integration cases. | The named gate, transition, Studio workflow, and sandbox-origin tests pass. This is local contract evidence only. |
...
| Repository hygiene | `git status --short --branch`, then `git fetch --filter=blob:none --depth=1 origin main:refs/remotes/origin/main`, then `git diff --check origin/main...HEAD` | Work happens on a dedicated branch, the intended committed branch-to-base diff is reviewable, and there is no whitespace error. |
| Secret and scope review | Review `git diff --stat origin/main...HEAD` and `git diff origin/main...HEAD`, or the pull-request Files changed tab, before approval. | No secret, environment file, production setting, deployment configuration, checkout behavior, or unrelated source file is included. |
...
|---|---|---|
| Preview-dependent end-to-end check | A ready Vercel Preview for the current PR head, followed by the PR’s E2E job completing against that preview. | Run the local responsive Home observation above. Record that it is local-only evidence. |
...
[PR #1984](https://github.com/groupthinking/EventRelay/pull/1984) remains the linked implementation reference for the Google sign-in retry work. Use its live GitHub checks as the source of truth for remote build, lint, security, preview, and end-to-end status.
...
[2]: https://github.com/groupthinking/EventRelay/pull/1984 "Google sign-in retry pull request"
