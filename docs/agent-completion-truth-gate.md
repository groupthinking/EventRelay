...
Each serialized run collects, evaluates, and uploads evidence before acquiring a status-publication lease. It fingerprints the canonical gate input and verdict, evaluator version, exact head, and publication outcomes. If both the bot comment and latest owned terminal status carry the same fingerprint, and the terminal status carries the expected state and explicit gate result, the run reuses them and writes zero statuses; this prevents the 15-minute sweep from exhausting GitHub's 1,000-status limit for one SHA and context. Changed evidence, a state transition, a deleted/stale comment, a finalizer status, or any other incomplete publication acquires a new pending lease. The workflow updates the PR comment before publishing the terminal status. A compare-and-swap check rejects superseded publication; an always-running finalizer turns publication failures into a failure status only when the run proves it owns the pending lease (or supersedes a predecessor), but that failure lacks the terminal-result marker and cannot be reused as a completed publication. Unknown ownership fails the workflow without touching the shared status, and the scheduled sweep supplies eventual recovery. Because evidence is collected before lease acquisition, a previous successful status can remain visible while changed evidence is being evaluated; lease creation failure can extend that window until recovery. Overwriting it without a proven lease could instead clobber a successor. These stale-green windows are why this status is advisory until #874 supplies independently head-bound enforcement. Actions are pinned to full commit SHAs. Only refresh-dispatch jobs receive actions: write.

Per-path focused-test proof currently parses terminal pytest outcomes from the exact-head trusted CI `test` job log. The parser associates a standalone terminal outcome with the most recent pytest node so live-log output between the node and `PASSED`/`FAILED` does not erase the result; a new node or pytest/Actions boundary clears that association. End-anchored parsing prevents parameter text from masquerading as an outcome, but code executed by pytest can deliberately forge console lines. The required independent review remains part of the trust boundary. Follow-up #874 must move per-path evidence to a machine-readable report emitted and published by the protected CI workflow before this signal is treated as hostile-agent-resistant.
...
The deterministic evaluator blocks, and the workflow run fails, if checkout, evidence collection, evaluation, evidence upload, comment publication, or final status publication fails. Every terminal status carries the immutable pending-status ID that acquired its publication lease and the full evidence fingerprint; status handoff compares owner IDs so a newer lease overrides a late predecessor while an older lease yields to a successor. Unknown or malformed ownership fails the run without publishing from an unproven lease. Because an older success can remain visible during that recovery window, this custom status alone is not fail-closed merge enforcement. The snapshot assumes repository write access and the default branch are trusted; organizations that delegate repository-write credentials to agents should move snapshot creation behind a protected environment or an independently authenticated GitHub App.
